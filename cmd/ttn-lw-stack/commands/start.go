...
var startCommand = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(cookieHashKey) == 0 || isZeros(cookieHashKey) {
			...
			logger.WithField("hash_key", cookieHashKey).Warn("No cookie hash key configured, generated a random one")
		}
		...
		if len(cookieBlockKey) == 0 || isZeros(cookieBlockKey) {
			...
			logger.WithField("block_key", cookieBlockKey).Warn("No cookie block key configured, generated a random one")
		}
